import { fetchMetricsData, fetchTopPathnames } from '@/backend/project-metric-data';
import {
  Card, Grid, Metric, Text, Title, Subtitle, Flex, AreaChart, BadgeDelta, CategoryBar, BarList } from "@tremor/react";
...
export async function MetricCardHolder({ session, project = null }: { session: Session | null, project?: string | null}) {
    ...
    return (
        ...
        <Grid numItemsMd={2} className="gap-2 mt-6">
  ...
  : { metric: string, email: string, project?: string | null })
{
    ...
    const barData = await fetchTopPathnames(email, metric, project);
    const processedBarData = metricInfo.ms2s? barData.map(data => {
            return {
                ...data,
                value: Math.round((data.value / 1000) * 100) / 100
            };
